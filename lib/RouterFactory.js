      ...
      .then(function(docs) {        
        if (docs.length > 0) {          
          var updates = docs.map(function(d) {
            return Model.findByIdAndUpdate(d._id, req.body.patch, { new: true }).exec()
              .then(function(doc) {return doc})              
              .catch(function(err) {return err})
          })                    
          ...
          Promise.all(updates)
            ...
            .catch(function(err) { return res.send({ status: 400, payload: deletes, msg: 'One or more update operation failed', query: conditions }) })
        } else {
          return res.send({ status: 404, msg: 'No docs found' })
        }
      })
